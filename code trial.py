...
from pymgrid.modules import BatteryModule, LoadModule
...
battery = BatteryModule(
    ...
    max_discharge=40,  # Maximum discharging rate in kW                                                                                                      
    efficiency=1.0,    # Battery efficiency
    init_soc=0     # Initial state of charge (50%)
)
...
class MicrogridState:
    ...
    def calculate_reward(self, control_dict):
        ...
        reward = -net_cost 
            ...
            # Penalty for having high SOC at end of peak period
            
        return reward, net_cost
        
    def min_cost(self, load_demand):
        """
        ...
        battery_energy = battery_capacity * 0          # Start at 50% SOC
        ...
        for hour in range(24):
            ...
            if is_peak:
                ...
                potential_discharge = min(40, battery_energy)
...
class QLearningAgent:
    ...
    def adaptive_epsilon_decay(self, current_cost, min_cost, tolerance=0.01):
        """
        Simple episode-based epsilon decay that slows down over time
        
        Args:
            current_cost: Not used but kept for compatibility
            min_cost: Not used but kept for compatibility
            tolerance: Not used but kept for compatibility
        """
        # Only decay if we're above the minimum epsilon
        if self.epsilon > self.epsilon_min:
            # Slower decay rate as episodes progress
            if self.episode_count < 1000:
                decay_rate = 0  # Faster decay early on
            elif self.episode_count < 5000:
                decay_rate = 0.9995  # Slower decay in middle stages
            else:
                decay_rate = 0.999  # Very slow decay in later stages
...
def plot_battery_actions_soc_comparison(env, battery_data, episodes_to_compare=[2500,6000,], save_path=None):
    """
    ...
    summary_text = (
        f"Learning Progress:\n"
        f"Optimal Range Time: {learning_metrics[2500]['optimal_range_time']:.1f}% → {learning_metrics[6000]['optimal_range_time']:.1f}%\n"
        f"Strategic Peak Discharges: {learning_metrics[2500]['peak_discharges']} → {learning_metrics[6000]['peak_discharges']}\n"
        f"Strategic Off-Peak Charges: {learning_metrics[2500]['off_peak_charges']} → {learning_metrics[6000]['off_peak_charges']}"
    )
...
def main():
    ...
    try:
        ...
        episodes_to_debug = [0, 2500, 5000]  # Debug start, middle, and end episodes
